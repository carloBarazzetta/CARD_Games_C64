...
;D:\DEV\CardGamesC64\CARD_Games_C64\Dev\Poker4\poker-4.prg ==1001==
  ...
  310 print"-----------------"
  320 print"{blu}si usa un mazzo di 52 carte."
  330 print"ognuno riceve 5 carte con le quali deve"
  340 print"formare una combinazione."
  350 print"le combinazioni possibili sono (in scala";
  360 print"crescente di valori):            "
  370 print"{red}coppia........(2 carte di ugual valore)"
  380 print"doppia coppia.(2 coppie)              "
  390 print"tris..........(3 carte di ugual valore)"
  400 print"scala.........(5 carte ordinate)       "
  410 print"full..........(1 tris e una coppia)    "
  420 print"colori........(5 carte di ugual seme)  "
  425 print"poker.........(4 carte di ugual valore)"
  430 print"scala reale...(una scala di ugual seme)"
  ...
  600 print"{clr}"
  610 print"{blk}     inserimento nomi dei giocatori"
  612 print"{blk}          gestiti dal computer"
  615 print:print
  620 forw=1to4
  625 n$(w)=""
  626 ifw<>4then630
  627 print"{clr}{down}{down}{down}{brn}inserisci il tuo nome >            <{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}";:goto640
  630 print"{red}nome del"w"{left}' giocatore >            <{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}";
  640 w5=1:print"{blu}";
  ...
  670 ifw5=13thengosub10500:goto650
 ...
 1071 ba=ba+25:gosub56000
 1072 forw=1to4
 1074 s(w)=s(w)-5:ifw=g1thens(w)=s(w)-5
 1076 gosub57005
 1078 next
 ...
 1803 printcl$;
 1805 ms=19:gj=4:gosub58000:print"{rvon}"n$(g);
 1810 ji=g:gosub52070
 1820 ms=12:gosub58000
 1830 g=g+1:nextw5
 1840 kk=4:gosub60000
 1850 printcl$;
...
12000 sl=5:poke650,128
12020 ms=16:gj=4:gosub58000:printsl;"{left}  {left}$ ";
12030 geta$:ifa$=""then12030
12035 ifa$<>"+"anda$<>"-"anda$<>chr$(13)thengosub10500:goto12030
12040 ifa$=chr$(13)then12080
12050 ifa$<>"-"then12070
12060 sl=sl-5:ifsl=0thensl=5:gosub10500
12065 goto12020
12070 sl=sl+5:ifsl>25thensl=sl-5:gosub10500
12075 goto12020
12080 poke650,0
12090 return
...
14010 rd=int(rnd(0)*5)*5-10+5*(ap=0)
14020 w9=int(pt(g))
14022 w8=0:forw=1to4:ifvs(w)=-1thenw8=w8+1
14024 next
14026 ifw8=2thenrd=rd-20*(rnd(0)<.8)
14030 w9=(w9+(w9=7)*4)*10:rd=w9+rd
14040 ifap<>0then14250
14100 ifrd<=0thenms=5:gosub58000:return:rem passa
14200 sl=rd:ifsl>25thensl=25
14205 ms=2:gosub58000:print"{rvon}";sl;"$":rem apre di sl
14210 s(g)=s(g)-sl:vs(g)=sl:ap=g:pu=sl:ba=ba+sl:rem sistema l'apertura
14220 goto14530
14250 ms=10:gj=g:gosub58000
14252 ifvs(g)>5then14260
14255 ifpu-rd>15thenms=1:gosub58000:kk=g:gosub60000:vs(g)=-1:return:rem lascia
14260 rd=rd-pu
14270 ifrd>=5then14400
14300 s(g)=s(g)-(pu-vs(g)):ba=ba+(pu-vs(g)):vs(g)=pu:rem gioca
14320 ms=3:gj=g:gosub58000
14330 goto14530
14400 ifrd>25thenrd=25
14405 pu=pu+rd:s(g)=s(g)-(pu-vs(g)):ba=ba+(pu-vs(g)):vs(g)=pu:rem rilancia
14420 ms=4:gosub58000:print"{rvon}";rd;"$":rem rilancia di rd
14530 gosub56000:gosub57000:return
15000 rem puntata finale
15010 gosub54000
15020 gj=g:ms=10:gosub58000
15030 rd=int(rnd(0)*5)*5-10
15040 w9=int(pt(g))
15050 rd=rd+w9*5-(w9>1)*w9*w9*5+10*(ap=0)+20*(w9=0)
15070 w8=0:forw=1to4:ifvs(w)=-1thenw8=w8+1
15080 next
15090 ifw8=2andap<>0thenrd=rd-20*(w9<>0)
15100 ifap<>0then15250
15110 ifrd<=0thenms=7:gosub58000:return:rem cip
15200 sl=rd:ifsl>25thensl=25-int(rnd(0)*2)*5
15205 ms=0:gosub58000:print"{rvon}";sl;"$":rem puntata
15210 s(g)=s(g)-sl:vs(g)=sl:ap=g:pu=sl:ba=ba+sl:goto15600:rem sistema puntata
15250 ms=10:gosub58000
15260 ifvs(g)>15then15280
15270 ifpu-rd>15thenms=1:gosub58000:kk=g:gosub60000:vs(g)=-1:return:rem lascio
15280 rd=rd-pu:ifw8=2andap<>0thenrd=rd+20*(w9<>0)
15290 ifrd>5then15400
15300 ms=6:gosub58000:s(g)=s(g)-(pu-vs(g)):ba=ba+(pu-vs(g)):vs(g)=pu:rem vedo
15310 goto15600
15400 ifrd>25thenrd=25-(int(rnd(0)*2)*5)
15405 pu=pu+rd:s(g)=s(g)-(pu-vs(g)):ba=ba+(pu-vs(g)):vs(g)=pu:rem rilancia
15420 ms=4:gosub58000:print"{rvon}";rd;"$":rem rilancia di rd
...
50030 forw=1to35
50035 gosub10000
50040 ps=int(rnd(0)*52+1)
50050 mz(0,1)=mz(ps,1):mz(0,2)=mz(ps,2)
50060 p1=int(rnd(0)*52+1)
...
54140 ifcl=1thenpt(w)=6+(p4+1)*.01+.001*p3:goto54500
...
54230 ift1<>0thenpt(w)=3+.01*t1-2*(k1<>0):goto54500
...
55090 return
56000 print"{rvon}{blk}{home}{down}{down}{down}{down}{down}{down}{down}{down}{down}{down}{down}{down}{down}{rght}{rght}{rght}{rght}puntata =      {left}{left}{left}{left}{left}{left}"pu"$"
56010 print"{rvon}{up}{up}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}{rght}piatto =       {left}{left}{left}{left}{left}{left}{left}"ba"$"
56020 return
57000 w=g
57005 ifw=4then57035
57010 print"{home}{down}";
57020 forw1=1tow*14-15+6-len(str$(s(w)))/2:print"{rght}";:next
57025 print"{rvon}{blu}         {left}{left}{left}{left}{left}{left}{left}{left}{left}"s(w)"$"
57030 goto57040
...
57038 print"{rvon}{blu}            {left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}{left}"s(4)"$";
57040 return
58000 print"{home}"
